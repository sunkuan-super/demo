...
object KafkaDemo {
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("KafkaDemo").setMaster("local[*]")
    val ssc = new StreamingContext(sparkConf, Seconds("6".toInt))
    ...
    val consumerStrategies = ConsumerStrategies.Subscribe(topic, kafkaParams)
    val kafkaDS = KafkaUtils.createDirectStream(ssc, LocationStrategies.PreferConsistent, consumerStrategies)
